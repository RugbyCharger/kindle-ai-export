...
async function main() {
  ...
  const concurrency = Number.parseInt(getEnv('CONCURRENCY') ?? '32', 10)
...
By ${authors.join(', ')}`
  ...
  let chapterCount = 0
  for (const { tocItem, text: rawText } of iterateChapters(
    metadata.toc,
    content
  )) {
    if (isPreview && chapterCount > 0) break
    chapterCount++
    ...
    // Combine successive paragraphs if they can fit within a single audio batch.
    let j = 0
    do {
      const chunk = t[j]!
